...
use twlinter::rules::ruleset::{CaseRule, Profile, RuleType, SpellingRule};
...
fn punct_definition_list_colon_skipped() {
    ...
    use twlinter::rules::ruleset::IssueType;
    let scanner = Scanner::new(vec![], vec![]);
...
fn grammar_clean_text_produces_no_issues() {
    ...
    let has_grammar = output
        .issues
        .iter()
        .any(|i| i.rule_type == twlinter::rules::ruleset::IssueType::Grammar);

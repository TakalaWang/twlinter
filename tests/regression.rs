...
use twlinter::engine::disambig::{disambiguate_batch, DisambigConfig};
...
fn load_scanner() -> (Scanner, S2TConverter) {
    let ruleset = twlinter::rules::loader::load_embedded_ruleset().unwrap();
...
fn working_text<'a>(s2t: &'a S2TConverter, text: &'a str) -> std::borrow::Cow<'a, str> {
    if twlinter::engine::zhtype::detect_chinese_type(text)
...
fn scan_and_fix(
    ...
    // Apply fixes.
    let excluded =
        twlinter::engine::scan::build_exclusions_for_content_type(work_text, content_type);
...
fn deterministic_corpus_stable_output() {
    ...
    for case in &corpus.cases {
        ...
        let active: Vec<_> = issues2
            .iter()
            .filter(|i| i.severity != twlinter::rules::ruleset::Severity::Info)
            .collect();
        assert!(
            active.is_empty(),

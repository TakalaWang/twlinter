// Criterion benchmarks for twlinter scanning pipeline.
//
...
use twlinter::engine::markdown::build_markdown_excluded_ranges;
...
fn bench_post_scan_transforms(c: &mut Criterion) {
    ...
    use twlinter::fixer::remap_to_post_fix;
    use twlinter::fixer::AppliedFix;
    use twlinter::rules::ruleset::{Issue, IssueType, Severity};
...
fn bench_cpu_attribution_100kb(c: &mut Criterion) {
    use twlinter::engine::scan::ContentType;
    use twlinter::engine::zhtype::detect_chinese_type;
    use twlinter::rules::ruleset::{PoliticalStance, ProfileConfig};
    ...
    let excluded =
        twlinter::engine::scan::build_exclusions_for_content_type(&text, ContentType::Plain);
    ...
    group.bench_function("build_exclusions_plain", |b| {
        b.iter(|| {
            let e = twlinter::engine::scan::build_exclusions_for_content_type(
                black_box(&text),
                ContentType::Plain,
            );
            black_box(&e);
    ...
    group.bench_function("lineindex_100kb", |b| {
        use twlinter::engine::lineindex::{ColumnEncoding, LineIndex};
...
fn bench_pipeline_breakdown(c: &mut Criterion) {
    ...
    let line_index = twlinter::engine::lineindex::LineIndex::new(&text);
    ...
    group.bench_function("detect_and_lineindex", |b| {
        b.iter(|| {
            let zh = twlinter::engine::zhtype::detect_chinese_type(black_box(&text));
            let idx = twlinter::engine::lineindex::LineIndex::new(black_box(&text));
            black_box(zh);
            black_box(&idx);
        });
    });

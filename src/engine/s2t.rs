...
//      one-to-many chars intentionally fall back to identity
...
mod tests {
    ...
    fn phrase_conversion() {
        ...
        // 作業系統 is a TWPhrases mapping that twlinter handles separately.
        assert_eq!(c.convert("操作系统"), "操作系統");
        // 内存→內存 (char-level only). 記憶體 is a TWPhrases mapping
        // handled by twlinter's cross_strait rules, not by s2t.
    ...
    fn high_risk_ambiguous_chars_are_excluded() {
        let ambiguous: std::collections::HashSet<char> =
            s2t_data::AMBIGUOUS_ST_CHARACTERS.iter().copied().collect();
        for ch in [
            '干', '复', '咸', '范', '丑', '佣', '伙', '舍', '症', '姜', '沈', '克', '后', '里',
        ] {
            assert!(ambiguous.contains(&ch), "{ch} must stay ambiguous");
        }
    }
